...
describe('TokenService', () => {
  ...
  beforeEach(() => {
    ...
    mockWalletService = {
      ...
      getWallet: jest.fn(() => ({
        transferTransaction: jest.fn().mockResolvedValue({ recipe: 'mock' }),
        proveTransaction: jest.fn().mockResolvedValue({ proven: 'mock' }),
        submitTransaction: jest.fn().mockResolvedValue('tx-identifier-123'),
      })),
    };
  ...
  describe('sendShieldedToken', () => {
    ...
    it('should mark transaction failed on error', async () => {
      ...
      mockWalletService.getWallet.mockReturnValue({
        transferTransaction: jest.fn().mockRejectedValue(new Error('Transfer failed')),
