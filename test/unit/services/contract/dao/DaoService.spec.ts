...
jest.mock('@lib/config/constants.js', () => ({
  DAO_CONFIG: {},
  TOKEN_CONFIG: {
    DEFAULT_DECIMALS: 6,
...
jest.mock('@services/providers/ProviderFactory.js', () => ({
  ProviderFactory: {
    createDaoProviders: (jest.fn() as any).mockResolvedValue({
      publicDataProvider: {
        queryContractState: (jest.fn() as any).mockResolvedValue({
          data: {
...
jest.mock('@contracts/dao/index.js', () => ({
  joinDaoVotingContract: (jest.fn() as any).mockResolvedValue({
    deployTxData: {
      public: {
    ...
    callTx: {
      open_election: (jest.fn() as any).mockResolvedValue({
        public: { txId: 'tx-open-123', blockHeight: 1000 },
      }),
      close_election: (jest.fn() as any).mockResolvedValue({
        public: { txId: 'tx-close-123', blockHeight: 1001 },
      }),
      cast_vote: (jest.fn() as any).mockResolvedValue({
        public: { txId: 'tx-vote-123', blockHeight: 1002 },
      }),
      fund_treasury: (jest.fn() as any).mockResolvedValue({
...
import { DaoService, DaoServiceConfig } from '@services/contract/dao/DaoService.js';
...
describe('DaoService', () => {
  ...
  describe('start', () => {
    it('should start and join DAO contract', async () => {
      ...
      const { ProviderFactory } = jest.requireMock('@services/providers/ProviderFactory.js') as any;
      const { joinDaoVotingContract } = jest.requireMock('@contracts/dao/index.js') as any;
  ...
  describe('openElection', () => {
    ...
    it('should log failure on error', async () => {
      ...
      const { joinDaoVotingContract } = jest.requireMock('../../../../../src/contracts/dao/index.js') as any;
      joinDaoVotingContract.mockResolvedValueOnce({
        deployTxData: { public: { contractAddress: '0x123' } },
        callTx: {
          open_election: (jest.fn() as any).mockRejectedValue(new Error('Election error')),
        },
      });
