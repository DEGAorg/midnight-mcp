...
describe('Config Module', () => {
  ...
  describe('loadConfig function', () => {
    it('should throw error if AGENT_ID is not provided', async () => {
      await expect(async () => {
        const { loadConfig } = await import('../../src/lib/config/env.js');
        loadConfig();
    ...
    it('should throw error when external proof server is enabled but required env vars are missing', async () => {
      ...
      await expect(async () => {
        const { loadConfig } = await import('../../src/lib/config/env.js');
        loadConfig();
      }).rejects.toThrow('When USE_EXTERNAL_PROOF_SERVER is true, PROOF_SERVER, INDEXER, INDEXER_WS, and MN_NODE must all be provided');
    });
  ...
  describe('getNetworkId helper', () => {
    it('should convert config NETWORK_ID to NetworkId enum', async () => {
      ...
      const { loadConfig, getNetworkId } = await import('../../src/lib/config/env.js');
      const config = loadConfig();
  ...
  describe('getWalletBackupFolder helper', () => {
    it('should return agent-specific wallet backup folder', async () => {
      ...
      const { loadConfig, getWalletBackupFolder } = await import('../../src/lib/config/env.js');
      const config = loadConfig();
