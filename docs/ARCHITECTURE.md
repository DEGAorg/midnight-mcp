...
// Per-request handling - extract agent ID from X-Agent-Id header
const agentId = request.headers['x-agent-id'];
const services = await sessionManager.getOrCreateSession({ agentId });
const mcpServer = new MCPServer(services);

// Create stateless transport (SessionManager handles session mapping)
...
- SessionManager handles automatic session mapping via X-Agent-Id header
- LRU cache with automatic session eviction
- Stateless HTTP transport (no session IDs generated by transport)
- Prometheus metrics
...
sequenceDiagram
    ...
    Client->>HTTP Server: POST /mcp (X-Agent-Id: agent-1)
...
**How Agent ID-Based Session Management Works:**

1. **Request arrives** with `X-Agent-Id` header (e.g., "agent-1")
2. **Validation** via `validateMcpRequest()` in request-validator.ts:
   - Checks header exists and is non-empty
   - Verifies agent is registered via `SeedManager.hasAgentSeed(agentId)`
   - Returns 401 if agent not found
3. **Session lookup** in SessionManager's internal Map<agentId, Session>:
   - If exists: returns cached services + updates last accessed time
   - If new: creates WalletOrchestrator, loads agent's seed, initializes services
4. **Session tracking** via Map maintains:
   - Service instances (WalletOrchestrator)
   - Creation timestamp
   - Last accessed timestamp
   - Session state (initializing/active/closing/closed)
5. **Automatic cleanup**:
   - Background task checks for idle sessions every 5 minutes
   - Evicts sessions idle longer than timeout (default 1 hour)
   - Evicts LRU session when max limit reached

**Key Design:**
- Agent ID = Session ID (simple 1:1 mapping)
- No separate session tokens or cookies needed
- Agent registration (seed file) acts as authentication
- Each agent gets isolated storage and services

```typescript
export class SessionManager {
  private sessions: Map<string, Session>;  // agentId -> Session
  ...
  async getOrCreateSession(options: CreateSessionOptions): Promise<ServiceDependencies> {
    ...
    if (existingSession) {
      existingSession.lastAccessedAt = new Date();  // Update LRU
      return existingSession.services;
    }

    // Validate agent registered
    if (!SeedManager.hasAgentSeed(agentId)) {
      throw new Error(`Agent '${agentId}' not registered`);
    }

    // Create new session with services
    const orchestrator = new WalletOrchestrator({ agentId });
    await orchestrator.start();
